...
example = function() {
  ...
  base_core = load_parcel(project.dir,"base","base_core")

  core = load_parcel(project.dir,"s2r","s2r_core")

  regcheck = core$regcheck %>% left_join(base_core$reg %>% select(cmd,step),by="step") %>% arrange(desc(deviation))
  head(regcheck)
...
regxvar = sp$base_regxvar$regxvar
regvar = sp$base_regvar$regvar
...
navigate_to_repex("',project.dir,'")
')
  si = sparcels$base_regsource$regsource
