...
Below are relevant files of my R package regtranslate. Can you check where the default ssc arguments for translation (possibly depending on the Stata command) are specified? How can be best adapt the code to set these defaults. In particular for areg to fixest translations we want as default:

ssc(K.fixef = "full")

Explain and then propose the concrete code changes.
