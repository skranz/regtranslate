...
Here we have a failure in an example:
...
> project_dir = "/home/rstudio/repbox/projects/aer_100_3_11"
> # runid = 167
> # Stata: replace male_beauty = 0 if male_beauty==.
> data = drf_load_data(project_dir, "cached_dta/167_cache.dta")
> data$stata2r_original_order_idx = seq_len(nrow(data))
> assign("has_original_order_idx", TRUE, envir = stata2r::stata2r_env)
> 
> # runid = 168
> # Stata: xtreg donation small_gift large_gift warm_vcm warm_small warm_large under30 elderly, i(id) mle
> pid = 168
...
> 
> # Don't call library(plm), would overload dplyr::lag
> rcmd = "plm::plm"
> cc_cols = c("small_gift", "large_gift", "warm_vcm", "warm_small", "warm_large", "under30", "elderly", "donation")
> dat = stata_drop_missing(dat, cc_cols)
> dat[['.plm.id']] = dat[['id']]
> dat = regtranslate::cterm_to_saveterm_cols(dat)
> formula = `donation` ~ `small_gift` + `large_gift` + `warm_vcm` + `warm_small` + `warm_large` + `under30` + `elderly`
> reg = plm::plm(formula = formula,data = dat,index = '.plm.id')
> sum = summary(reg)
> ct = broom::tidy(sum,conf.int=TRUE)
Error: No tidy method for objects of class summary.plm
> # This works
...
1 small_g…   0.0396     0.655    0.0604 0.952     -1.24 
2 large_g…   2.07       0.678    3.05   0.00236    0.738
3 warm_vcm   3.07       1.13     2.73   0.00649    0.865
4 warm_sm…   0.323      0.955    0.338  0.735     -1.55 
5 warm_la…   0.886      1.00     0.883  0.377     -1.08 
6 under30   -1.50       0.693   -2.16   0.0309    -2.86 
7 elderly   -0.977      0.517   -1.89   0.0591    -1.99 
# ℹ 1 more variable: conf.high <dbl>
> glance = broom::glance(reg)
> ct$term = regtranslate::saveterm_to_cterm(ct$term)
...
I just don't know why for plm the summary was computed and

> ct = broom::tidy(sum,conf.int=TRUE)

Called, are there possibly instances were we need the summary?

If you don't know the reason or there are such cases, maybe for the plm translation try first to call broom::tidy on reg and only if there is an error try it on the summary. But only for plm translations, not generally...
